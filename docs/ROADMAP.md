...
| G4 Markdown 교환·검토 UI | evidence/revision/wikilink를 포함하는 deterministic Markdown export와 review candidate 전용 safe text import를 제공한다. canonical memory 자동 승격이나 외부 vault write 없이 Tauri review/graph/query/exchange 설정 패널을 기존 UI 스타일에 맞춰 추가한다. | export가 반복 실행해도 같은 결과를 내고 evidence/revision이 보존된다. import는 제한된 Markdown 형식과 현재 source identity/evidence를 검증해 후보로만 저장하며 malformed text와 임의 외부 쓰기를 거부한다. 기존 UI 상태·스타일과 localStorage를 보존한다. | Core 교환은 text-only이며 UI의 선택 파일 읽기·다운로드·복사/붙여넣기를 제공한다. Core는 파일 경로·외부 vault 쓰기를 받지 않는다. 실제 native Tauri window, Keychain, iCloud/FSEvents, live provider, WebGL은 이 작업으로 증명하지 않는다. |

상태 문서에는 단계별 change ID와 실행한 검증을 기록한다. 구현 계획에 없는
기능은 이 확장 작업에 포함하지 않으며, native/live 경계는 fixture·in-memory
테스트의 통과와 분리해 보고한다.

2026-09-21: G1–G4 로컬 구현과 통합 검증 완료. 단계별 증거와 native/live 미검증 범위는 [검증 기록](./MEMORY_GRAPH_VALIDATION.md)을 참고한다.
...
- 이 확장 작업에서는 사용자가 명시적으로 요청한 부모의 로컬 병합만 예외로
  허용한다. 원격 push와 릴리스 공개는 계속 범위 밖이다.
...
## 업무·할 일·일정·대시보드 확장 — 2026-09-23

상태: 계획 작성 완료, 구현 미착수. 상세 범위와 단계별 체크리스트는
[업무 워크플로우 로드맵](WORKFLOW_ROADMAP.md)을 따른다.

- 목표: 미팅 일정 → 회의록 → 테크스펙 → 작업 티켓 → GitHub PR → 티켓 완료 → 작업 기록을 하나의 업무로 연결하고, 오늘의 일정·할 일·다음 행동을 대시보드와 알림으로 제공한다.
- 첫 출시: W0 계약/기술 검증 → W1 업무·할 일 → W2 대시보드 → W3 일정·알림 → W4 티켓·PR·완료 기록 → W5 실제 환경 인수 검증.
- 후속: W6 근거 기반 AI 정리 지원, 별도 범위 확정 후 W7 선택적 외부 쓰기. 첫 출시는 기존 읽기 중심 연결과 로컬 쓰기 경계를 유지한다.
- 기존 native/live 출시 검증을 대체하지 않는다. 구현 상태와 검증 증거는 단계 완료 시 별도로 기록한다. Jev 검토보다 먼저 진행하며 출시 날짜는 W0 결과로 산정한다.

## 최후순위 작업 검토: Jev

- 상태: 도입 여부 검토 후보. 구현 확정이나 MVP 완료 조건이 아니다.
- 순서: 위 출시 검증 및 기존 후속 범위를 모두 우선하며, 그 이후 가장 마지막에 검토한다.
- 범위: 선택적 검색 결과 재정렬을 먼저 평가하고, 효과가 확인되면 메모 재검토 후보 선별을 검토한다.
- 검토 기준: 한국어·한영 혼합 질문 50~100개로 기존 검색 대비 상위 결과의 유용성·지연·비용을 비교하고, 외부 전송 조건과 장애 시 로컬 기능 유지를 확인해 도입·보류·미도입을 기록한다.
- 상세: [Jev 도입 검토](JEV_REVIEW.md). 기존 사용자 검토·근거·권한 경계를 유지한다.

## Jira 개인 범위 및 Confluence 확장 — 2026-09-22

현재 사용자 요청에 따라 개인 Jira/상위 티켓 및 Confluence 읽기 연결을 구현한다. 상세 작업 분담·완료 기준은 [구현 계획](ATLASSIAN_IMPLEMENTATION_PLAN.md), 검증 결과는 [구현 상태](IMPLEMENTATION_STATUS.md)에 기록한다. 최근 열람 기록·OAuth·원격 자동 갱신은 후속 범위다.
