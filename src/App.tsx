...
import { PageMotion, SegmentedControl } from "./ui/AppComponents";
import assistantLogo from "./assets/aidebook-assistant-logo.png";

const GraphCanvas = lazy(() => import("./GraphCanvas"));

type Page = "dashboard" | "workflow" | "work" | "home" | "notes" | "activity" | "graph" | "search" | "connections" | "settings";
...
function Icon({ name }: { name: "work" | "notes" | "activity" | "graph" | "panel-left" | "panel-right" | "search" | "settings" | "link" | "close" }) {
  const paths = {
...
function NavButton({ active, icon, onClick, children }: { active: boolean; icon: "work" | "notes" | "activity" | "graph"; onClick: () => void; children: ReactNode }) {
  return <button className={`navbtn ${active ? "active" : ""}`} type="button" aria-current={active ? "page" : undefined} onClick={onClick}><Icon name={icon} />{children}</button>;
}
...
function App() {
  ...
  const [workflowSelection, setWorkflowSelection] = useState<{kind: "work" | "task"; id: string} | undefined>();

  function pageTitle() {
    if (page === "work") return `작업 묶음　/　${works[work]}`;
    return { dashboard: "개인 작업 공간 / 오늘의 업무", workflow: "개인 작업 공간 / 업무와 할 일", home: "개인 작업 공간　/　최근 맥락", notes: "개인 작업 공간　/　모든 메모", activity: "개인 작업 공간　/　활동 기록", graph: "탐색　/　지식 그래프", search: "전체 검색", connections: "설정　/　연결 상태", settings: `설정　/　${setting}` }[page];
  }

  function renderPage() {
    if (page === "dashboard") return <DashboardPanel reducedMotion={graphMotionReduced} onOpen={(selection) => { setWorkflowSelection(selection); navigate("workflow"); }} />;
    if (page === "workflow") return <WorkflowPanel initialSelection={workflowSelection} reducedMotion={graphMotionReduced} />;
  ...
  function NoteCard({ note, selected: isSelected, onClick }: { note: Note; selected: boolean; onClick: () => void }) {
    return <Card asChild size="2" variant="surface"><button className={`note ${isSelected ? "selected" : ""}`} type="button" aria-pressed={isSelected} onClick={onClick}><div className="note-top"><span className="type">{note.kind}</span><span className="mono">{note.time}</span></div><h3>{note.title}</h3><p>{note.body}</p><div className="note-foot"><span>{note.author}</span><span>·</span><span>근거 {note.sources.length}개</span>{note.kind === "후보" && <span>· 검증 전</span>}</div></button></Card>;
  }
  ...
  return <div className={`shell ${sidebarHidden ? "sidebar-hidden" : ""} ${sidebarMobileOpen ? "sidebar-mobile-open" : ""} ${inspectorHidden ? "inspector-hidden" : ""}`}><aside className="sidebar" aria-label="주요 메뉴"><div className="brand"><img src={assistantLogo} alt="aidebook" /></div><nav className="main-nav" aria-label="주요 메뉴"><NavButton active={page === "dashboard"} icon="work" onClick={() => navigate("dashboard")}>오늘의 업무</NavButton><NavButton active={page === "workflow"} icon="work" onClick={() => { setWorkflowSelection(undefined); navigate("workflow"); }}>업무와 할 일</NavButton><NavButton active={page === "home"} icon="work" onClick={() => navigate("home")}>최근 맥락</NavButton><NavButton active={page === "notes"} icon="notes" onClick={() => navigate("notes")}>모든 메모<span className="count">{notes.filter((note) => !note.retracted).length}</span></NavButton><NavButton active={page === "activity"} icon="activity" onClick={() => navigate("activity")}>활동 기록</NavButton><NavButton active={page === "graph"} icon="graph" onClick={() => navigate("graph")}>그래프<span className="count">{activeWorkNotes.length}</span></NavButton></nav><div className="works"><p className="navlabel">작업 묶음</p><div>{works.map((name, index) => <button className={`navbtn work ${page === "work" && work === index ? "active" : ""}`} type="button" key={name} aria-current={page === "work" && work === index ? "page" : undefined} onClick={() => chooseWork(index)}><span className="dot" /><span>{name}</span><span className="count">{notes.filter((note) => note.work === index && !note.retracted).length}</span></button>)}</div></div><div className="sidebar-bottom"><button className={`navbtn ${page === "connections" ? "active" : ""}`} type="button" onClick={() => navigate("connections")}><Icon name="link" />연결 상태</button><button className={`navbtn ${page === "settings" ? "active" : ""}`} type="button" onClick={() => navigate("settings")}><Icon name="settings" />설정<span className="count">⌘ ,</span></button><div className="profile"><span className="avatar">나</span><div>개인 작업 공간<div className="small muted">이 기기에 보관</div></div></div></div></aside><main className="main" id="content"><header className="topbar"><button className="tool" type="button" aria-label="사이드바 접기 또는 펼치기" aria-expanded={!sidebarHidden} onClick={() => { if (window.innerWidth <= 760) setSidebarMobileOpen((open) => !open); else setSidebarHidden((hidden) => !hidden); }}><Icon name="panel-left" /></button><div className="crumb">{pageTitle()}</div><button className="search-launch" type="button" onClick={() => navigate("search")}><Icon name="search" />자료와 메모 검색 <kbd>⌘ K</kbd></button><Badge className="app-badge" variant="surface" color={page === "workflow" || page === "dashboard" ? "blue" : "gray"} radius="medium">{page === "workflow" || page === "dashboard" ? "로컬 업무" : "예시 데이터"}</Badge>{(page === "work" || page === "graph") && <button className="tool inspector-toggle" type="button" aria-label="메모 근거 패널" aria-expanded={!inspectorHidden} onClick={() => setInspectorHidden((hidden) => !hidden)}><Icon name="panel-right" /></button>}</header><div id="view" tabIndex={-1}><PageMotion pageKey={`${page}-${page === "work" || page === "graph" ? work : ""}`} reducedMotion={graphMotionReduced}>{renderPage()}</PageMotion></div></main>{editor && EditorModal()}{detail && DetailModal()}{toast && <div className="toast" role="status">{toast}{undoStack.length > 0 && <button type="button" onClick={undoChange}>되돌리기</button>}</div>}</div>;
