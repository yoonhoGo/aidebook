...
export default function PluginsPanel() {
  ...
  return <div className="page plugin-page" aria-busy={busy || undefined}><section className="section overview-section plugin-overview"><div className="container">
    ...
    {secretId && <form className="card plugin-token-form plugin-form" aria-labelledby="plugin-oauth-secret-title" onSubmit={saveSecret}>
      <h2 id="plugin-oauth-secret-title">Jira OAuth 앱 비밀키</h2>
      <label>Client secret<input type="password" autoComplete="new-password" required value={secret} onChange={e => setSecret(e.target.value)} /></label>
      <p className="small muted">연결별 macOS Keychain에 저장합니다.</p>
      <div className="plugin-form-actions"><button className="btn btn-primary" disabled={!native || busy}>비밀키 저장</button><button className="btn btn-ghost" type="button" disabled={!native || busy} onClick={() => { setSecret(""); setSecretId(null); }}>취소</button></div>
    </form>}
    {oauthId && <section className="card plugin-token-form plugin-form" aria-labelledby="plugin-oauth-pending-title" role="status">
      <h2 id="plugin-oauth-pending-title">OAuth 승인 대기 중</h2>
      {oauthCode && <p>GitHub 인증 코드: <strong>{oauthCode}</strong></p>}
      <p>브라우저에서 승인을 마치면 이 화면에 결과가 표시됩니다.</p>
    </section>}
    {removeId && <section className="card plugin-remove-confirmation" aria-labelledby="plugin-remove-title">
      ...
      <label className="plugin-check-row"><input type="checkbox" checked={deleteCredential} onChange={e => setDeleteCredential(e.target.checked)} />이 연결의 저장된 인증 정보도 삭제 (gh 로그인은 유지)</label>
