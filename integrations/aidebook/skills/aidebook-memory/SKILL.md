...
For personal Jira scope use `jira_scope: "mine"`, optionally `jira_include_reporter` and `jira_include_parents`. Existing project connections retain project scope. For Confluence use `confluence_mode: "authored"`, `"watched"`, or `"selected"`. `plugins.confluence.search` returns candidates without indexing; persist explicitly selected IDs in `confluence_page_ids` using `plugins.update`, then refresh. Never infer a selection from search results alone.
